...
/* ── Helper: read avatar from stored user ── */
...
function Dashboard() {
  ...
  const [topAvatar, setTopAvatar] = useState(() => getStoredAvatar());

  /* ── Listen for avatar update event ── */
  useEffect(() => {
    const handleAvatarUpdate = () => {
      setTopAvatar(getStoredAvatar());
    };
  ...
  const navItems = allNavItems.filter(item => accessibleModules.includes(item.key));
    ...
    <>
      ...
      <div style={styles.container}>
        ...
        <div style={{
          ...
          <div
            style={{
              ...styles.logoRow, ...styles.logoRowGlass,
              transform: headerHovered ? 'translateY(-2px)' : 'translateY(0)',
              boxShadow: headerHovered ? styles.logoRowHover.boxShadow : styles.logoRowGlass.boxShadow,
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={styles.logoTitle}>Spartan BTY</p>
              <div style={styles.userInfo}>
                <span style={styles.roleBadge}>{user?.role?.toUpperCase()}</span>
              </div>
            </div>
            {isMobile && (
              <button style={styles.closeBtn} onClick={(e) => { e.stopPropagation(); setSidebarOpen(false); }}>✕</button>
            )}
          </div>
          ...
          <nav style={styles.nav}>
            ...
            {navItems.map((item) => (
              <div
                ...
                style={{
                  ...styles.navItem,
                  backgroundColor: location.pathname === item.path ? '#ffffff' : hovered === item.label ? 'rgba(255,255,255,0.15)' : 'transparent',
                  borderLeft:      location.pathname === item.path ? '4px solid #fff' : '4px solid transparent',
                  transform:       hovered === item.label && location.pathname !== item.path ? 'translateX(4px)' : 'translateX(0)',
                  transition: 'all 0.2s ease',
                }}
              >
                <span style={styles.navIcon}>{item.icon}</span>
                <span style={{ fontWeight: location.pathname === item.path ? '700' : '400' }}>{item.label}</span>
          ...
          <div style={styles.topbar}>
            <div style={styles.topbarLeft}>
              ...
              <div>
                ...
                <p style={styles.pageSubtitle}>Welcome back, {user?.full_name}!</p>
              </div>
            </div>
              ...
              <div className="user-dropdown-container" style={styles.dropdownWrapper}>
                ...
                >
                  ...
                  <div style={{
                    ...
                    backgroundColor: topAvatar ? 'transparent' : '#c4607a',
                    color: '#fff', overflow: 'hidden',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: '15px', fontWeight: '700', flexShrink: 0,
                  }}>
                    {topAvatar
                      ? <img src={topAvatar} alt="avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      : initials
                    }
                  </div>
                  ...
                  {!isMobile && (
                    <>
                      ...
                      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#888" strokeWidth="2"
                        ...
                        style={{ transition: 'transform 0.2s', transform: userDropdownOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>
                        <path d="M6 9l6 6 6-6" />
                      </svg>
                    </>
                  )}
...
const styles = {
  container: {
    ...
    fontFamily: 'Segoe UI, sans-serif', backgroundColor: '#f5f0fa', boxSizing: 'border-box',
  },
  mobileOverlay: { position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 998 },
  sidebar: {
    width: '260px',
    ...
    boxShadow: '4px 0 15px rgba(0,0,0,0.1)', zIndex: 100, boxSizing: 'border-box',
  },
  logoRow: { display: 'flex', alignItems: 'center', gap: '12px', padding: '20px 16px', borderBottom: '1px solid rgba(255,255,255,0.2)' },
  logoRowGlass: {
    background: 'rgba(255, 255, 255, 0.75)', backdropFilter: 'blur(12px)',
    borderRadius: '16px', margin: '16px', padding: '16px',
    border: '1px solid rgba(255, 255, 255, 0.5)',
    boxShadow: '0 4px 16px rgba(196, 96, 122, 0.12)', cursor: 'pointer',
  },
  logoRowHover: { boxShadow: '0 6px 24px rgba(196, 96, 122, 0.2)' },
  logoContainer: {
    width: '48px', height: '48px', borderRadius: '12px',
    background: 'linear-gradient(135deg, rgba(255,95,147,0.1), rgba(255,141,181,0.1))',
    display: 'flex', alignItems: 'center', justifyContent: 'center',
    flexShrink: 0, border: '1px solid rgba(255, 95, 147, 0.2)',
  },
  ...
  closeBtn: {
    backgroundColor: 'rgba(255,255,255,0.25)', border: 'none', color: '#302e2e',
  ...
  navItem: {
    ...
    padding: '12px 20px', color: '#302e2e', cursor: 'pointer',
    fontSize: '14px', borderRadius: '0 8px 8px 0',
    marginRight: '12px', position: 'relative', boxSizing: 'border-box',
  },
  navIcon: { fontSize: '18px', minWidth: '24px', textAlign: 'center' },
  activeDot: { position: 'absolute', right: '12px', fontSize: '8px', color: '#302e2e' },
  main: { flex: 1, minHeight: '100vh', padding: '28px', maxWidth: '100%', overflowX: 'hidden', boxSizing: 'border-box' },
  topbar: {
    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
    marginBottom: '28px', backgroundColor: '#fff', padding: '16px 24px',
    borderRadius: '14px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)',
    minHeight: '72px', flexShrink: 0, gap: '12px',
    width: '100%', maxWidth: '100%', boxSizing: 'border-box',
  },
  topbarLeft: { display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 },
  hamburgerBtn: {
  ...
  hamburgerLine: { display: 'block', width: '18px', height: '2px', backgroundColor: '#c4607a', borderRadius: '2px' },
  ...
  topbarDivider: { width: '1px', height: '32px', backgroundColor: '#eee', flexShrink: 0 },
  ...
  topUserInfo: { display: 'flex', flexDirection: 'column', gap: '2px' },
  userName: { fontSize: '13px', fontWeight: '600', color: '#302e2e', margin: 0, whiteSpace: 'nowrap' },
  userRole: { backgroundColor: '#fdf0f3', color: '#c4607a', padding: '1px 8px', borderRadius: '12px', fontSize: '10px', fontWeight: '600', alignSelf: 'flex-start' },
  dropdownMenu: {
  ...
  dropdownDivider: { height: '1px', backgroundColor: '#eee', margin: '4px 0' },
  contentWrap: { width: '100%', maxWidth: '100%', overflowX: 'hidden', boxSizing: 'border-box' },
};
